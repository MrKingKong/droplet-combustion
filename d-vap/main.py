...
if __name__ == '__main__':
    ...
    endtime = 100e-6
    mdot_fuel = 10000*1.98e-6 #this number should be calculated based on equivalence ratio, kg/s
    ...
    for i in range(1,int(endtime/dt)):
        ...
        droplets.extend(entrainer(gas_env.T, mdot_fuel))  
...
#        print(str(mdot) + str(qdot) + str(gas_kernel.T)+'  '+str(len(droplets)))
    ...
    ax2.set_ylabel('X_O & X_CO', color='green', FontSize=15)
    ax2.plot([t*1e3 for t in time], X_CO2, color = 'blue', linewidth = 3)
    fig.legend(['X_fuel', 'X_O', 'X_CO2'], loc = 'left')
